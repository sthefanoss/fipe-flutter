import 'package:flutter/foundation.dart';

class ResourceImages {
  const ResourceImages();

  final jpg = const _JPGs();
  final svg = const _SVGs();
}

class _JPGs {
  const _JPGs();
  final String carExample = 'assets/jpg/ford_ka.jpg';
  final String drawerBackground = 'assets/jpg/drawer_background.jpg';
}

class _SVGs {
  const _SVGs();
  final String car = 'assets/svg/ford_ka.jpg';
  final String copyrightSymbol = 'assets/svg/copyright_symbol.svg';
  final String menuUnfold = 'assets/svg/menu_unfold.svg';
  final String menuFold = 'assets/svg/menu_fold.svg';
  final String logo = 'assets/svg/mob_car_logo.svg';
  final String starEmpty = 'assets/svg/star_empty.svg';
  final String starFilled = 'assets/svg/star_filled.svg';
}
